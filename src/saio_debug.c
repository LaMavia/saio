  ...
  for (suffix = 0;; suffix++) {
    ...
    foreach (lc, private->steps) {
      ...
      fprintf(f, "%04d %10.4f  %10.4f  %1d  %6d\n", i++, step->temperature,
              step->cost, step->move_result, step->joinrels_built);
    }
    fclose(f);
    break;
  }
...
static void dump_query_tree_node(QueryTree *tree, QueryTree *selected1,
                                 QueryTree *selected2, List *lselected1,
                                 List *lselected2, Relids relids, bool costs,
  ...
  if (costs)
    ...
    fprintf(f, "<br/><font point-size=\"7\">%.2f</font>", previous_cost);
  fprintf(f, ">];\n");
}

static void dump_query_tree_edge(Relids relids, Relids other_relids, FILE *f) {
  fprintf(f, "        \"(");
  fprintf_relids(f, relids);
  fprintf(f, ")\" -> \"(");
...
static Relids dump_query_tree_rec(PlannerInfo *root, QueryTree *tree,
  ...
  if (tree->left == NULL) {
    ...
    dump_query_tree_node(tree, selected1, selected2, lselected1, lselected2,
                         relids, costs, private->previous_cost, f);
  ...
  relids_left = dump_query_tree_rec(root, tree->left, selected1, selected2,
                                    lselected1, lselected2, costs, f);
  relids_right = dump_query_tree_rec(root, tree->right, selected1, selected2,
                                     lselected1, lselected2, costs, f);
...
void dump_query_tree_list(PlannerInfo *root, QueryTree *tree,
  ...
  dump_query_tree_list2(root, tree, selected1, selected2, selected, NIL, costs,
                        path);
}

void dump_query_tree_list2(PlannerInfo *root, QueryTree *tree,
                           QueryTree *selected1, QueryTree *selected2,
                           List *lselected1, List *lselected2, bool costs,
                           char *path) {
#ifndef SAIO_DEBUG
  return;
#else
  ...
  dump_query_tree_rec(root, tree, selected1, selected2, lselected1, lselected2,
                      costs, f);
  fprintf(f, "}\n");
  fclose(f);
#endif
