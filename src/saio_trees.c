 ...
 */
static List *merge_trees(PlannerInfo *root, List *result, QueryTree *tree,
                         bool force) {
  ListCell *prev;
  ListCell *lc;
...
static List *get_all_nodes_rec(QueryTree *tree, List *res) {
  if (tree == NULL)
    return res;
 ...
 */
List *get_all_nodes(QueryTree *tree) { return get_all_nodes_rec(tree, NIL); }

/* Recursive helper for get_parents */
static List *get_parents_rec(QueryTree *tree, bool reverse, List *res) {
  Assert(tree != NULL);
...
#ifdef NOT_USED
static void cleanup_tree(QueryTree *tree) {
  if (tree->left == NULL) {
    Assert(tree->right == NULL);
    return;
  }
