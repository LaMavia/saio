...
bool desirable_join(PlannerInfo *root, RelOptInfo *outer_rel,
...
void initialize_random_state(PlannerInfo *root, double seed) {
  ...
  memcpy(private->random_state, &seed,
...
bool join_can_be_legal(PlannerInfo *root, Relids relids1, Relids relids2,
  ...
  foreach (l, root->join_info_list) {
    ...
    if (bms_is_subset(sjinfo->min_lefthand, relids1) &&
       ...
       *		(a LJ b on Pab) LJ c ON Pbc = a LJ (b LJ c ON Pbc) on
       * Pab since joining B directly to C violates the lower SJ's RHS. We
       * assume that make_outerjoininfo() set things up correctly so that we'll
       * only match to some SJ if the join is valid. Set flag here to check at
       * bottom of loop.
       *----------
       */
      if (sjinfo->jointype != JOIN_SEMI &&
