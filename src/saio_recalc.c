...
static bool list_walker(List *l, bool (*walker)(QueryTree *, bool, void *),
...
static bool keep_costs(QueryTree *tree, bool fake, void *extra_data) {
...
static bool remove_from_planner(QueryTree *tree, bool fake, void *extra_data) {
  ...
  if (root->join_rel_hash != NULL) {
    if (hash_search(root->join_rel_hash, &(rel->relids), HASH_REMOVE, NULL) ==
        NULL)
...
static bool reset_memory(QueryTree *tree, bool fake, void *extra_data) {
...
static bool nullify(QueryTree *tree, bool fake, void *extra_data) {
  RelOptInfo **rel = fake ? &tree->tmp : &tree->rel;
...
static void rebuild_join_rel_hash(PlannerInfo *root) {
  HTAB *hashtab;
  HASHCTL hash_ctl;
  ListCell *l;
  ...
  hashtab =
      hash_create("JoinRelHashTable", 256L, &hash_ctl,
  ...
  foreach (l, root->join_rel_list) {
    ...
    hentry = (JoinHashEntry *)hash_search(hashtab, &(rel->relids), HASH_ENTER,
                                          &found);
    Assert(!found);
    hentry->join_rel = rel;
  }
...
static bool switch_contexts(QueryTree *tree, bool fake, void *extra_data) {
...
static bool recalculate(QueryTree *tree, bool fake, void *extra_data) {
  PlannerInfo *root = (PlannerInfo *)extra_data;
  ...
  private =
      (SaioPrivateData *)GetPlannerInfoExtensionState(root, saio_extension_id);
  ...
  *tree_rel = rel;
   ...
   */
  if (!compare_costs(root, private->previous_cost, SAIO_COST(rel),
                     private->temperature)) {
    return false;
  }
#endif
...
static void get_abc_paths(QueryTree *tree, QueryTree *tree1, QueryTree *tree2,
                          List **ab, List **c) {
  List *p1, *p2;
  List *a, *b;
...
static bool check_possible_join(QueryTree *tree, bool fake, void *extra_data) {
  PlannerInfo *root = (PlannerInfo *)extra_data;
...
static bool clean_possible_join(QueryTree *tree, bool fake, void *extra_data) {
  if (tree->tmp == NULL)
    return true;
...
static bool joins_are_possible(PlannerInfo *root, QueryTree *tree1,
                               QueryTree *tree2, List *ab, List *c) {
  bool ok;
...
static int swap_and_recalc(PlannerInfo *root, QueryTree *tree, QueryTree *tree1,
                           QueryTree *tree2) {
  List *ab, *c;
  bool ok;
  int move_result;
...
static saio_result saio_recalc_step(PlannerInfo *root, QueryTree *tree,
                                    List *all_trees) {
  List *choices, *tmp;
...
static void init_tree_contexts(QueryTree *tree) {
  Assert(tree != NULL);

  tree->ctx = AllocSetContextCreate(
      CurrentMemoryContext, "SAIO node", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
  tree->tmpctx = AllocSetContextCreate(
      CurrentMemoryContext, "SAIO tmp node", ALLOCSET_DEFAULT_MINSIZE,
      ALLOCSET_DEFAULT_INITSIZE, ALLOCSET_DEFAULT_MAXSIZE);
...
static void saio_recalc_initialize(PlannerInfo *root, QueryTree *tree) {
  SaioPrivateData *private =
      (SaioPrivateData *)GetPlannerInfoExtensionState(root, saio_extension_id);
  bool ok;
...
static void saio_recalc_finalize(PlannerInfo *root, QueryTree *tree) {
  SaioPrivateData *private =
      (SaioPrivateData *)GetPlannerInfoExtensionState(root, saio_extension_id);

  /* restore join_rel_list and join_rel_hash */
  root->join_rel_list = list_truncate(root->join_rel_list, private->savelength);
  root->join_rel_hash = private->savehash;
}

SaioAlgorithm algorithm = {.step = saio_recalc_step,
