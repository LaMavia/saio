...
#define EXT_NAME "saio"

PG_MODULE_MAGIC_EXT(.name = EXT_NAME, .version = PG_VERSION);
...
int saio_extension_id = -1;

/* Saved hook value in case of unload */
static join_search_hook_type prev_join_search_hook = NULL;

static RelOptInfo *saio_main(PlannerInfo *root, int levels_needed,
                             List *initial_rels) {
  if (saio_extension_id < 0) {
    saio_extension_id = GetPlannerExtensionId(EXT_NAME);
  }
...
void _PG_init(void) {
  ...
  DefineCustomBoolVariable("saio", "Use SA for query planning.", NULL,
                           &enable_saio, true, PGC_USERSET, 0,
                           SAIO_GUC_HOOK_VALUES);

  DefineCustomRealVariable("saio_seed", "SA random seed.", NULL, &saio_seed,
                           0.0, 0.0, 1.0, PGC_USERSET, 0, SAIO_GUC_HOOK_VALUES);

  DefineCustomIntVariable("saio_equilibrium_factor",
                          "SA scaling factor for reaching equilibirum.", NULL,
                          &saio_equilibrium_factor, 16, 1, INT_MAX, PGC_USERSET,
  ...
  DefineCustomIntVariable("saio_moves_before_frozen",
                          ...
                          &saio_moves_before_frozen, 4, 1, INT_MAX, PGC_USERSET,
                          0, SAIO_GUC_HOOK_VALUES);
  /* Install hook */
